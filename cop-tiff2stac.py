...
PRODUCTS = ["forestcover","grasscover","imperviousness","water-wetness"]
...
def list_products(bucket, http_url, prefix):
    def get_date(image):
        ...
        fdate=datetime.strftime(next(matches),"%Y")
        try:
            sdate=datetime.strftime(next(matches),"%Y")
            return "fdate_sdate"
...
def stac_file_path(date_range):
    return "item/CLMS_{}.json".format(date_range)
