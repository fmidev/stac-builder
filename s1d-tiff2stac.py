...
#PRODUCTS = ["vv","vh"]
PRODUCTS = ["vv","vh","vv_min","vv_max","vv_mean","vv_std","vvvh_mean","vh_min","vh_max","vh_mean","vh_std"]
...
def stac_file_path(date_range):
    return "item/S1DM_{}.json".format(date_range)
...
def tiff_to_stac(item_file_name, dates, products, baseurl):
    ...
    identifier = "S1DM_{}".format(dates)
    title = "Sentinel 1 daily mosaics - {}".format(dates)
    ...
    time_end = datetime.strptime(dates.split("-")[0], '%Y%m%d')
