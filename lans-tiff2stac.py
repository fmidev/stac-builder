...
PRODUCTS = ["b1", "b2", "b3", "b4", "b5","b7","ci","fc","hv","nc"]
...
def list_products(bucket, http_url, prefix):
    def get_date(image):
        matches=datefinder.find_dates(image)
        fdate=datetime.strftime(next(matches),"%Y%m%d")
        try:
            sdate=datetime.strftime(next(matches),"%Y%m%d")
            return "fdate_sdate"
...
def stac_file_path(date_range):
    return "item/LSM_{}.json".format(date_range)
...
def tiff_to_stac(item_file_name, dates, products, baseurl):
    ...
    identifier = "LSM_{}".format(dates)
    title = "Landsat mosaics - {}".format(dates)
...
if __name__ == '__main__':
    ...
    for dates, products in tiffs.items():
        ...
        if not overwrite and item_filename[-3:] in ['tif','vrt'] and os.path.isfile(item_file_name):
